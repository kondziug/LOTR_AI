...
class MacroSimulator(BaseSimulator):
    ...
    def setAgents(self, params):
        if rlMode[0] == 'l':
            if pipeline == 4: self.agent_p = Agent('planning', 3, params['lr'], params['lr'])
            elif pipeline == 5: self.agent_p = QAgent('planning', 3, params['lr'])
        if rlMode[1] == 'l':
            if pipeline == 4: self.agent_q = Agent('questing', 3, params['lr'], params['lr'])
            elif pipeline == 5: self.agent_q = QAgent('questing', 3, params['lr'])
