...
class Node:
    ...
    def createBoard(self):
        ...
        for card in self.data['Board']['questDeck']:
            questDeck.addCard(globals.dictOfCards[card], 1)
        ...
        for card in self.data['Board']['encounterDeck']:
            encounterDeck.addCard(globals.dictOfCards[card], self.data['Board']['encounterDeck'][card])
        ...
        if self.data['Board']['activeLand']:
            landToAdd = globals.dictOfCards[next(iter(self.data['Board']['activeLand']))].copy()
    ...
    def createPlayer(self):
        ...
        for card in self.data['Player']['playerDeck']:
            playerDeck.addCard(globals.dictOfCards[card], self.data['Player']['playerDeck'][card])
        heroes = []
        for card in self.data['Player']['heroes']:
            cardToAdd = globals.dictOfCards[card].copy()
    ...
    def createStaticArea(self, owner, name):
        ...
        for card in self.data[owner][name]:
            for _ in range(0, self.data[owner][name][card]):
                area.append(globals.dictOfCards[card].copy())
        return area
